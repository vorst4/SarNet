...
class ResNetAE5(nn.Module, ABC):
    ...
    def __init__(self):
        ...
        self.decoder = nn.Sequential(
            Reshape((-1, kernels, 1, 1)),
            TransposeResNetBlock(kernels, kernels//2),
            TransposeResNetBlock(kernels//2, kernels//4),
            TransposeResNetBlock(kernels//4, kernels//8),
            TransposeResNetBlock(kernels//8, kernels//16),
            nn.ConvTranspose2d(kernels//16, kernels//16, kernel_size=2, stride=2, padding=0, bias=True),
            nn.ReLU(),
            nn.Conv2d(kernels//16, 1, kernel_size=3, stride=1, padding=1, bias=True))
