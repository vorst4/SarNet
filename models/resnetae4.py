...
class ResNetAE4(nn.Module, ABC):
    ...
    def __init__(self):
        ...
        self.identity = nn.Conv2d(1, 1, kernel_size=3, stride=2, padding=1, bias=True)
        self.decoder2 = nn.Sequential(
            nn.ConvTranspose2d(kernels[4]+1, kernels[4], kernel_size=2, stride=2, padding=0, bias=True),
            nn.ReLU(),
    ...
    def forward(self, input_img, input_meta):
        ...
        x = self.bottleneck(x)
        x = self.decoder1(x)
        i = self.identity(input_img)
        x = self.decoder2(torch.cat((i, x), dim=1))
        return x
