...
class Down(nn.Module, ABC):
    ...
    def __init__(self, ci, co, ri, sq=True, ex=True):
        super().__init__()
        d = settings.dropout_rate
        self.res1 = blk.InvResBlock(ci=ci, co=co, k=3, dropout=d,
                                    ri=ri,
                                    expand=ex,
                                    squeeze=sq)
        self.res2 = blk.InvResBlock(ci=co, co=co, k=3, dropout=d,
                                    ri=ri,
                                    expand=ex,
                                    squeeze=sq)
    ...
    def forward(self, x):
        return self.down(self.res2(self.res1(x)))


class Up(nn.Module, ABC):

    def __init__(self, ci, co, ri, mode='transpose', sq=True, ex=True):
        super().__init__()
        d = settings.dropout_rate
        self.res1 = blk.InvResBlock(ci=ci, co=co, k=3, dropout=d,
                                    ri=ri,
    ...
    def forward(self, x):
        return self.up(self.res2(self.res1(x)))


class SarNetRNcSe(nn.Module, ABC):
    lr_ideal = 1e-5
